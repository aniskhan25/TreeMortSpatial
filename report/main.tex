...
\twocolumn[
  \begin{center}
    ...
    beyond this host structure, mortality retains a \emph{modest but genuine residual aggregation at
    the sub-kilometre scale} ($\lesssim$1\,km): the inhomogeneous $L$-function lies above a
    host-conditioned simulation envelope even under a richer null (spruce\,+\,total volume\,+\,stand
    edge), a local-contagion signature on top of host availability. We establish these claims with a
    ...
    disturbance-agent attribution is withheld. Tree-level estimation, a variance-stabilised
    treatment of scales beyond 1\,km, and a marked-process test against an external disturbance-agent
    layer are the natural next steps.
    \end{abstract}
    \vspace{0.3in}
  \end{center}
]
...
\begin{enumerate}
  \item \textbf{Host-conditioned spatial characterisation of boreal canopy mortality.}
        Treating 698,223 individual dead-tree detections as a point process and conditioning on a
        pre-mortality forest-inventory host surface, we show that mortality intensity tracks host
        availability (it rises with both spruce and total growing-stock volume) and yet retains a
        modest, genuine \emph{residual aggregation at the sub-kilometre scale} ($\lesssim$1\,km)
        that survives a richer host-conditioned null (spruce\,+\,total volume\,+\,stand edge): a
  ...
  \item \textbf{A falsification-and-validation harness for morphometrics from black-box detectors.}
        ...
        covariance-preserving null with mixture-model discreteness diagnostics, and purposive
        cluster-sample inference (effective sample size = the 7 sampling blocks). Applied here it
  ...
  \item \textbf{A demonstration that cluster shape alone does not attribute disturbance agent.}
        ...
        signal beyond hull shape. We therefore characterise spatial structure rather than infer
...
\emph{inhomogeneous} null whose first-order intensity is driven by host. The host surface is the
...
inhomogeneous-Poisson patterns ($L_{\rm inhom}\!-\!r\approx0$). At the 6\,km tile scale the ring
estimator is reliable to $r\lesssim1$\,km ($\approx$ window/4); larger scales require a
variance-stabilised treatment (future work). We run two independent implementations: a tile-pooled
inhomogeneous pair-correlation/$L$ (intensity normalised per tile), and a per-block $L_{\rm inhom}$
in \texttt{spatstat} with the intensity fitted globally (pooled Poisson across forest cells) and
host-conditioned envelopes. Analyses use cluster centroids; the tree-level process is a stated next
step.
...
range. Against a spruce-only host null, the inhomogeneous $L$-function lies far above the
host-conditioned envelope across 100\,m--1\,km (Figure~\ref{fig:hostcond}a). Crucially, the signal
...
(Figure~\ref{fig:hostcond-ss}). The two implementations agree on the qualitative finding---residual
sub-kilometre aggregation beyond host, present in every block---and on the robustness of
spruce-driven host-tracking; they differ in the \emph{magnitude} of the residual (the per-block
cumulative $L$ accumulates more multi-scale clustering than the tile-pooled pair-correlation), so we
report the existence and sub-kilometre scale of the residual, not a single magnitude. We interpret it
cautiously: it is established at the centroid scale on a 7-block sample to $r\lesssim1$\,km; the
tree-level process and scales beyond 1\,km remain for future work.

\begin{figure}[htb]
  \centering
  \includegraphics[width=\linewidth]{host_conditioned_spatstat.pdf}
  \caption{Independent \texttt{spatstat} confirmation: per-block inhomogeneous $L_{\rm inhom}(r)-r$
           (observed, solid) vs the global-host-conditioned simulation envelope (shaded), for each of
           the seven super-site blocks. Observed lies above the envelope across 250\,m--1\,km in every
           block, confirming residual sub-kilometre aggregation beyond host.}
  \label{fig:hostcond-ss}
\end{figure}
...
\begin{figure}[htb]
  ...
  \caption{Residual mortality aggregation beyond forest host. Inhomogeneous $L_{\rm inhom}(r)-r$
           ...
           as total volume absorbs broader host structure). Valid to $r\lesssim1$\,km for the 6\,km
           tile windows.}
  \label{fig:hostcond}
\end{figure}
...
\mypar{Mortality tracks host, then aggregates beyond it.}
...
volume---yet retains a modest, genuine residual aggregation at the sub-kilometre scale that survives
...
absorbed most of the apparent excess, so the defensible statement is a \emph{bounded} residual at
$\lesssim1$\,km, not the order-of-magnitude clustering a naive (homogeneous-null) reading suggested.
...
\mypar{Future work.}
Three steps would consolidate the host-conditioned result. First, a variance-stabilised
point-process treatment in \texttt{spatstat}: tree-level (rather than centroid) intensity, the
inhomogeneous $K$/$L$ with proper large-window edge correction to extend reliably beyond 1\,km, and
the fitted-intensity model as a formal \texttt{ppm} with simulation envelopes. Second, a
marked-process test against an external disturbance-agent layer would move from characterising
structure to attributing it; we note one obstacle for the wind-versus-beetle question specifically:
the publicly available pan-European attribution products \citep{senf2021mapping} group wind and bark
beetle into a single combined class, so a dedicated regional agent layer (e.g.\ Metsäkeskus damage
records) would be required. Third, multi-temporal imagery, feasible from the same pipeline for prior
years, would date mortality and turn the static host-conditioned pattern into a space--time process,
testing directly whether this year's foci predict next year's mortality---the definitive contagion
test. Beyond this, external validation that breaks the circularity of a shape-only study would
overlay a species layer from Finland's multi-source national forest inventory and an independent
