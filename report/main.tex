...
% Cross-reference the Supplementary Information (compile supplementary.tex first so its .aux exists).
\usepackage{xr}
...
Correlation analysis (Supplementary Fig.~\ref{fig:si-corr}) shows strong positive covariation
among size metrics ($n$, area) and expected trade-offs between compactness and aspect
ratio. Supplementary Fig.~\ref{fig:scatters} presents key pairwise scatter plots.
...
Supplementary Fig.~\ref{fig:typology_map}; the inset locates this window within the national sample.
...
tail (Supplementary Fig.~\ref{fig:sizedist}), but this follows directly from its larger mean cluster area
...
quantity, and between-band (latitudinal) structure is unsampled rather than absent. LISA maps (Supplementary Fig.~\ref{fig:lisa}) illustrate the spatial
...
isolated singletons, so the graph is sparse. Supplementary Fig.~\ref{fig:contagion} shows connectivity
...
(range 0.973--0.978 across 5 spatial folds; Supplementary Fig.~\ref{fig:shap}); re-running the cross-validation
