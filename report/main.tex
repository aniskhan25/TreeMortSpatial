...
\twocolumn[
  \begin{center}
    ...
    which we summarise---as an interpretive simplification rather than a discovered boundary---by
    two endpoint classes: an \textit{elongated/dispersed} type (23.5\% of clusters) and a
    \textit{compact/aggregated} type (76.5\%). This structure exceeds a covariance-preserving
    null distribution (single multivariate Gaussian matched to the observed feature covariance;
    $p=0.001$), confirming it is not an artefact of correlated marginals; however, mixture-model
    selection (BIC) favours more than two components and the primary morphological axis is
    unimodal, indicating a continuum rather than a clean dichotomy. A held-out Random Forest
    with spatial block cross-validation reproduces the partition (AUC\,=\,0.976\,$\pm$\,0.002),
    but we show this reflects geometric redundancy among shape features rather than independent
    corroboration: the only shape-orthogonal feature (landscape connectivity) adds no
    discriminative information. Four stability gates, run by re-clustering from raw, show that the
    \emph{discrete} two-class reading is not robust---the split is not invariant to the clustering
    radius, its prevalence is an artefact of the aspect-ratio filter, it carries no directional
    signal beyond hull shape, and it is partly entangled with detector behaviour---whereas the
    underlying compactness--elongation \emph{gradient} is stable. We therefore report the gradient,
    not a typology, and deliberately avoid disturbance-agent labels: associations with wind, bark
    beetle, or other agents are not supported by these data, which lack the species, dating, and
    ground-truth layers required for attribution.
    \end{abstract}
    \vspace{0.3in}
  \end{center}
]
...
Bonferroni-corrected significance threshold $\alpha^* = 0.05/20 = 0.0025$.
...
\begin{figure}[htb]
  ...
  \caption{Distributions of key cluster metrics across all 14,582 clusters.
           All metrics exhibit right-skewed or moderately skewed distributions.}
...
metrics at five distance thresholds. Significant spatial autocorrelation (Bonferroni
$\alpha^* = 0.0025$) is present across most metric--threshold combinations, with
values generally declining as the threshold increases from 200\,m to 5000\,m.
The scale-dependence of autocorrelation implies that morphological similarity between
clusters operates primarily at stand to landscape scales ($< 2$\,km) rather than at
the regional scale. LISA maps (Figure~\ref{fig:lisa}) illustrate the spatial structure
of fractal-dimension autocorrelation within the study-region display window; labels are computed
on the full survey dataset to preserve correct neighbourhood weights.

\begin{figure*}[htb]
  \centering
  \includegraphics[width=\linewidth]{fig_moran_heatmap.png}
  \caption{Moran's $I$ heatmap (E6): four metrics (rows) $\times$ five distance
           thresholds (columns). Bonferroni-corrected significance threshold
           $\alpha^* = 0.0025$. Significant cells are marked with an asterisk.
           Autocorrelation is strongest at the 200--500\,m scale and attenuates at
           distances $\geq 2$\,km.}
  \label{fig:morans}
\end{figure*}
...
\begin{figure}[htb]
  ...
  \caption{SHAP beeswarm plot for the held-out Random Forest (E10). Compactness
           is the dominant predictor; reach\_200m (shape-independent connectivity)
           ranks last.}
  \label{fig:shap}
\end{figure}
