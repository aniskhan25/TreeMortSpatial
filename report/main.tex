...
latitudinally-distinct regimes. It is not. Stratified into southern ($\sim$60--62\textdegree{}N,
$n=8{,}988$), central ($\sim$64--65\textdegree{}N, $n=4{,}217$), and northern
($\sim$67--68\textdegree{}N, $n=1{,}377$) bands, the defining gradient axis is essentially
identical within each band (compactness--aspect-ratio correlation $r = -0.864$, $-0.867$, $-0.868$
respectively), as are the endpoint medians. The compactness--elongation gradient is thus a
within-regime morphological phenomenon reproduced at every latitude, not a between-band regime
mixture.
...
quantity, and between-band (latitudinal) structure is unsampled rather than absent. LISA maps (Figure~\ref{fig:lisa}) illustrate the spatial
...
the 312 tiles form a discontinuous set of study blocks spanning the latitudinal gradient rather
than a probability sample of the boreal zone; reported distributions are conditional on tile
placement, and the tile-selection design should be documented in any operational use. We did,
however, verify that the central gradient finding is reproduced within each latitude band (above),
so the principal result does not depend on the relative weighting of regions in the sample.
