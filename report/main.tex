...
\twocolumn[
  \begin{center}
    ...
    processes that generate them. We present a national-scale analysis of 698,223 dead-tree
    locations identified from a 2023 aerial-imagery sample distributed across Finland (312 tiles,
    $\sim$60.4--67.8\textdegree{}N). Using DBSCAN
...
applied to high-resolution aerial imagery acquired in 2023. The detections come from 312 aerial
...
surveyed tiles and should not be read as an exhaustive national census.
...
The spatial distribution of all 14,582 cluster centroids across the sampled tiles is shown in
Figure~\ref{fig:spatial_extent}.

\begin{figure}[htb]
  \centering
  \includegraphics[width=\linewidth]{fig1_spatial_extent.png}
  \caption{Map of dead-tree cluster centroids ($n=14{,}582$) sampled across Finland
           ($\sim$60.4--67.8\textdegree{}N; EPSG:3067), in discrete latitudinal bands.}
...
Figure~\ref{fig:typology_map}; the inset locates this window within the national sample.
...
\begin{figure}[htb]
  ...
  \caption{LISA hotspot map for fractal dimension in the southern display window. Labels
...
We have presented a national-scale morphological analysis of 14,582 dead-tree clusters
identified from a 2023 aerial-imagery sample distributed across Finland (312 tiles, southern
boreal zone to Lapland). Using DBSCAN clustering,
